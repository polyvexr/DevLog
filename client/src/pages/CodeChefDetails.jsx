...
export default function CodeChefDetails() {
  ...
  const confirmUnlink = async () => { setUnlinkDialogOpen(false); try { await unlinkPlatform("codechef"); navigate("/"); } catch (err) { alert(err.response?.data?.message || "Failed to unlink platform"); } };
  ...
  if (error || !data) return (
    <div className="bg-white border border-slate-200 p-12 text-center rounded-xl space-y-6 max-w-lg mx-auto mt-12">
      ...
      <p className="text-slate-500 text-xs font-mono max-w-xs mx-auto leading-relaxed">This account is not yet connected. Connect your CodeChef profile to monitor statistics.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 space-y-12">
      <div className="space-y-4">
        ...
        <PlatformDetailsHeader platform="codechef" username={data.username} icon={SiCodechef} title="CodeChef" onUnlink={handleUnlink} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl space-y-2">
          <div className="text-[8px] font-mono font-semibold uppercase tracking-wider text-slate-400">Star Rating</div>
          <div className="text-3xl font-mono font-bold leading-none" style={{ color: getStarColor(stats.stars) }}>{"★".repeat(stats.stars || 1)}</div>
        </div>
        <StatBox label="Total Solved" value={stats.totalSolved || 0} />
      </div>
      <ContestHistoryList contests={stats.ratingHistory?.slice().map(c => ({ contestName: c.contestName || c.contestCode, rank: c.rank, rating: c.rating, date: c.date }))} platform="codechef" />
      <DisconnectDialog open={unlinkDialogOpen} platform="codechef" onConfirm={confirmUnlink} onCancel={() => setUnlinkDialogOpen(false)} />
    </div>
  );
}
