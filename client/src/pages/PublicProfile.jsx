...
const platformConfig = {
  ...
  github: { icon: SiGithub, colorClass: "text-slate-700", label: "GitHub", url: u => `https://github.com/${u}` },
...
export default function PublicProfile() {
  ...
  if (error) return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-700 flex items-center justify-center p-6 text-center select-none">
      <div className="bg-white border border-slate-200 p-12 text-center rounded-xl space-y-6 max-w-lg mx-auto">
        ...
        <h1 className="text-xl font-[Cormorant_Garamond] font-semibold italic text-slate-900">Access Denied</h1>
        <p className="text-slate-500 text-xs font-mono max-w-xs mx-auto leading-relaxed">{error}</p>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-700 font-sans pb-32 overflow-x-hidden select-none">
      <div className="max-w-7xl mx-auto px-6 pt-16 space-y-12">
        
        {/* Profile Identity Card */}
        <div className="bg-white border border-slate-200 rounded-xl p-8 md:p-12 relative overflow-hidden flex flex-col md:flex-row items-center gap-8 md:gap-12">
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div className="space-y-1.5">
              ...
              <h1 className="text-4xl md:text-5xl font-[Cormorant_Garamond] font-light italic text-slate-900 tracking-tight leading-none">
                {user.name}
              </h1>
              <p className="font-mono text-[9px] text-slate-400 flex items-center justify-center md:justify-start gap-1">
                <FiCalendar /> Joined {new Date(user.memberSince).toLocaleDateString("en-US", { month: 'long', year: 'numeric' })}
              </p>
            </div>

            {user.bio && (
              <p className="text-slate-500 text-xs italic max-w-xl border-l-2 border-[#e23e2d]/50 pl-4 py-1 text-left">
                "{user.bio}"
              </p>
            )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {platforms.map((p) => {
              const cfg = platformConfig[p.platform] || { icon: FiActivity, colorClass: "text-slate-500", label: p.platform, url: () => "#" };
              const Icon = cfg.icon;
              return (
                <div key={p.platform} className="bg-white border border-slate-200 hover:border-slate-300 p-6 rounded-xl transition-all space-y-6 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-[Cormorant_Garamond] font-semibold italic text-slate-800 text-lg leading-tight">{cfg.label}</h3>
                          <p className="font-mono text-[9px] text-slate-400 uppercase tracking-wider">@{p.username}</p>
                        </div>
                      </div>
                      <a
                        ...
                        className="p-2 bg-slate-100 border border-slate-200 rounded text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-200">
                    {getDisplayStats(p).map((stat, si) => (
                      <div key={si} className="bg-slate-100 border border-slate-200 rounded p-3 space-y-1">
                        <div className="text-[8px] font-mono font-semibold uppercase tracking-wider text-slate-400 truncate">
                          {stat.k}
                        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.badges.map((b, i) => (
                <div key={i} className="bg-white border border-slate-200 p-5 rounded-xl flex items-center gap-4">
                  <div className="w-10 h-10 rounded bg-slate-100 border border-slate-200 flex items-center justify-center text-xl flex-shrink-0">
                    <span>{b.icon || "🎖️"}</span>
                  </div>
                  <div className="min-w-0">
                    <h4 className="font-mono text-[9px] font-semibold text-slate-700 uppercase tracking-wider truncate">{b.label}</h4>
                    <p className="font-mono text-[8px] text-slate-400 uppercase tracking-wider mt-0.5">{b.platform}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <footer className="pt-24 pb-8 text-center space-y-6 border-t border-slate-200">
          <Link to="/" className="inline-flex items-center gap-1.5 font-[Cormorant_Garamond] font-semibold italic text-slate-900 text-xl hover:text-slate-600 transition-colors">
            <FiZap className="text-[#e23e2d] text-base" /> DevLog
          </Link>
          <div className="flex justify-center gap-6 font-mono text-[8px] text-slate-400 uppercase tracking-wider">
...
function MetricBox({ label, value, icon }) {
  return (
    <div className="bg-white border border-slate-200 p-6 rounded-xl flex justify-between items-center transition-all hover:border-slate-300">
      <div className="space-y-1">
        <div className="text-[9px] font-mono font-semibold uppercase tracking-wider text-slate-400">{label}</div>
        <div className="text-3xl font-mono font-bold text-slate-900">{value || "0"}</div>
      </div>
...
function StatInfo({ icon, text, link }) {
  const content = (
    <div className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 transition-colors text-xs font-mono cursor-pointer">
