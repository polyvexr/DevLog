...
export default function LinkPlatform() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to link platform. Try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen">
      ...
      <main className="p-8">
        <div className="app-container">
          ...
          <form onSubmit={submit} className="max-w-lg glass-card-hover p-8 rounded-2xl" aria-live="polite">
            {error && <div className="mb-4 text-sm text-red-400" role="alert">{error}</div>}
            <div className="mb-6">
              <label htmlFor="platform" className="block text-sm font-semibold text-gray-300 mb-3">Platform</label>
              <select
                id="platform"
            ...
            <div className="mb-8">
              <label htmlFor="username" className="block text-sm font-semibold text-gray-300 mb-3">Username</label>
              <input
                id="username"
                value={username}
                type="text"
            ...
            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 rounded-lg font-bold text-lg hover:shadow-lg hover:shadow-blue-500/50 transition-all duration-200 disabled:opacity-60">
